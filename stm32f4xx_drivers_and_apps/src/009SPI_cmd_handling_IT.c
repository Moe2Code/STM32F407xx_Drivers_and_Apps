...
int main (void)
{
	...
	//Enabling and setting priority for SPI2 IRQNumber at the NVIC

	SPI_IRQInterruptConfig(IRQ_NO_SPI2, ENABLE);
	SPI_IRQPriorityConfig(IRQ_NO_SPI2, NVIC_IRQ_PRI14);
	...
	while(1)
	{
		...
		SPI_ReceiveDataIT(&SPI2Handle, &dummy_read, 1);
		...
		SPI_ReceiveDataIT(&SPI2Handle, &ackbyte, 1);
		...
		if(SPI_VerifyResponse(ackbyte))
		{
			...
			SPI_SendDataIT(&SPI2Handle, args, 2);
		...
		if(SPI_VerifyResponse(ackbyte))
		{
			...
			SPI_SendDataIT(&SPI2Handle, args, 1);
			...
			SPI_ReceiveDataIT(&SPI2Handle, &analog_read, 1);
		...
		if(SPI_VerifyResponse(ackbyte))
		{
			...
			SPI_SendDataIT(&SPI2Handle, &dummy_write, 1);
			SPI_ReceiveDataIT(&SPI2Handle, &Led_status, 1);
		...
		uint8_t message[] = "This is the third SPI application!";
		...
		if(SPI_VerifyResponse(ackbyte))
		{
			...
			// send message
			SPI_SendDataIT(&SPI2Handle, message, args[0]);
		...
		if(SPI_VerifyResponse(ackbyte))
		{
			for(i=0; i<10; i++)
			{
				...
				SPI_ReceiveDataIT(&SPI2Handle, board_ID+i, 1);
			}

			board_ID[11] = '\0';   // null termination of array (required by %s format specifier)
